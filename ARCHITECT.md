...
### IntelligentAudio.NeuralEngine (The Core Engine)

**Purpose:** High-performance inference and command mapping.
**Technology:** ONNX Runtime (.NET) for Intent-to-Command translation.

**Responsibility:**
    - **Inference Pipeline:** Consumes text from Whisper.net and runs ONNX models to resolve specific musical/technical intents.
    - **Discovery & Mapping:** Uses `DiscoveryService` to dynamically map resolved intents to available parameters in connected software (e.g., VST3 parameters, CC-mappings).
    - **Agnostic Execution:** Logic must remain decoupled from specific endpoints; it maps "Increase Reverb" to a discovered parameter ID, regardless of the destination.
    **Constraints:**
    - **Inference Optimization:** ONNX models must be loaded into memory once; use pinned buffers for input/output tensors to avoid heap churn.
    - **Dynamic Discovery:** Parameter mapping must be cached after discovery to ensure O(1) lookup during the hot path.
